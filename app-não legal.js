...
function abrirModalAdmin() {
    // 1. Configurações de Palco
    const inputDelay = document.getElementById("input-delay-partida");
    if (inputDelay) {
        inputDelay.value = appStorage.configGlobais.delayPartida || 0;
    }

    // 2. Abre o modal adicionando a classe 'active'
    document.getElementById("modal-admin-container").classList.add("active");

    // 3. Atualiza labels de contexto
    const lblLista = document.getElementById("txt-nome-lista-backup");
    if (lblLista) {
        lblLista.innerText = `"${appStorage.listaAtiva}"`;
    }

    // 4. Preenche o select de ordenação (o antigo seletor)
    ...
    // 5. CHAVE: Preenche a nova estrutura de listas gerenciáveis (a div com botões)
    // Esta função foi a que ajustamos anteriormente para usar o container-listas-gerenciaveis
    popularSeletorGerenciarListas();
}

// Abre o modal de ordenação
function abrirModalOrdenacao(nomeLista) {
    document.getElementById('modal-ordenar-container').classList.add('active');
    ...
    ids.forEach((id, idx) => {
        ...
        item.draggable = true; // Habilita drag and drop
        item.dataset.id = id;
        item.innerHTML = `
            <span class="biblioteca-icone">≡</span>
            <span class="biblioteca-nome">${idx + 1}. ${escapeHtml(musica.titulo)}</span>
        `;

        // Lógica simples de Drag and Drop
...
function salvarNovaOrdem(nomeLista) {
    const itens = document.querySelectorAll('#container-lista-ordenacao .biblioteca-item');
    appStorage.listas[nomeLista] = Array.from(itens).map(it => it.dataset.id);
    localStorage.setItem('gelcifras_db', JSON.stringify(appStorage));
    sincronizarEAAplicarInterface();
}
...
let listaSelecionadaAdmin = ""; // Adicione isso perto das outras variáveis globais no topo do arquivo

// Remova qualquer duplicata desta função no seu app.js e mantenha esta versão única:
function popularSeletorGerenciarListas() {
    const container = document.getElementById('container-listas-gerenciaveis');
    if (!container) return;
    ...
    if (chaves.length === 0) {
        container.innerHTML = '<div style="padding:10px; color:var(--text-muted); text-align:center;">Nenhuma lista criada</div>';
        return;
    }

    chaves.forEach(nomeLista => {
        const item = document.createElement('div');
        item.className = 'biblioteca-item'; 
        
        item.innerHTML = `
            <span class="biblioteca-nome">${escapeHtml(nomeLista)}</span>
            <button class="btn-pequeno" onclick="abrirModalOrdenacao('${nomeLista.replace(/'/g, "\\'")}')">
                ↕ Ordenar
            </button>
        `;
        
        item.onclick = () => {
            listaSelecionadaAdmin = nomeLista; 
            document.querySelectorAll('.biblioteca-item').forEach(i => {
                i.style.borderColor = 'var(--border-color)';
                i.style.background = 'var(--card-bg)';
            });
            item.style.borderColor = 'var(--chord-color)';
            item.style.background = 'var(--sub-panel-bg)';
        };
...
function excluirLista() {
    const nome = listaSelecionadaAdmin; // Usa a variável que definimos no clique
    
    if (!nome) { 
        mostrarToast('Selecione uma lista na lista acima primeiro.'); 
        return; 
    }
    ...
    listaSelecionadaAdmin = ""; // Reseta a seleção
    fecharModalAdmin();
    sincronizarEAAplicarInterface();
}
...
// Certifique-se de que estas funções só aparecem UMA VEZ no seu arquivo:

function duplicarLista() {
    if (!listaSelecionadaAdmin) { mostrarToast('Selecione uma lista primeiro.'); return; }
    document.getElementById('modal-prompt-titulo').innerText = `Duplicar: ${listaSelecionadaAdmin}`;
    document.getElementById('modal-prompt-input').value = `${listaSelecionadaAdmin} (cópia)`;
    document.getElementById('modal-prompt-acao').value = 'duplicar';
    document.getElementById('modal-prompt-lista').classList.add('active');
    setTimeout(() => document.getElementById('modal-prompt-input').select(), 100);
}
...
function abrirModalRenomearLista() {
    if (!listaSelecionadaAdmin) { mostrarToast('Selecione uma lista primeiro.'); return; }
    document.getElementById('modal-prompt-titulo').innerText = `Renomear: ${listaSelecionadaAdmin}`;
    document.getElementById('modal-prompt-input').value = listaSelecionadaAdmin;
    document.getElementById('modal-prompt-acao').value = 'renomear';
    document.getElementById('modal-prompt-lista').classList.add('active');
    setTimeout(() => document.getElementById('modal-prompt-input').select(), 100);
}
...
function confirmarAcaoPromptLista() {
    const acao = document.getElementById('modal-prompt-acao').value;
    const novoNome = document.getElementById('modal-prompt-input').value.trim();
    const nomeOriginal = listaSelecionadaAdmin;

    if (!novoNome) { alert('Nome vazio.'); return; }
    if (appStorage.listas[novoNome] && novoNome !== nomeOriginal) {
        alert(`Já existe lista "${novoNome}".`); return;
    }
    ...
    if (acao === 'duplicar') {
        ...
        appStorage.listas[novoNome] = appStorage.listas[nomeOriginal];
        delete appStorage.listas[nomeOriginal];
        if (appStorage.listaAtiva === nomeOriginal) appStorage.listaAtiva = novoNome;
